"""
*This script is the same as 'make_clim_bam.py just with a smaller time domain. It is an initial test for the 
BAM calculations.
...
years_arr = np.arange(1985,1987)
years = go_stats.make_np_list(years_arr)
#months = ['01','02','03','04','05','06','07','08','09','10','11','12']
months = ['03','04','05']
days_send = go_stats.make_np_list(np.arange(1,32))
...
def save_nc(mast_u, mast_v, mast_t, lat_send, lev, fname):
    #* 8-23 I changed this function to now include an fname call to work with the make_bam_climo.py script
    # I didn't change any of the other make_clim_bam scripts, the thought was this smal test version
    # was largely useless now.
    os.chdir('/uufs/chpc.utah.edu/common/home/mace-group6/merra_monthly_data/BAM/data4bam')
    #f = cdf.Dataset('mam_climatological_mean_1985_1987.nc4', 'w', format='NETCDF4')
    f = cdf.Dataset(fname, 'w', format='NETCDF4')
...
#* This is my first guess on daily analysis into climatlogical mean, going to save as a seasonal average.
    for year in years:
        for month in months:
            ...
            for day in days:
                ...
                lat_hold, lon_hold, lev_hold, time_hold, tot_u_hold, tot_v_hold, t_hold = bam_merra_calc_smal.call_main(month, year, day)
                if lat_hold[0] == 'n':
	...
	# year == years[0] for non-djf generation.
        if year == years[1]:
