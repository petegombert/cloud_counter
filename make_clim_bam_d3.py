...
dimension. This will make generating the monthly BAM numbers less resource intensive.
...
import requests
import bam_merra_calc_d3
import go_stats

years_arr = np.arange(1992,2003)
...
def save_nc(mast_u, mast_v, mast_t, lat_send, lev):
    ...
    f = cdf.Dataset('jja_climatological_mean_2003_2013.nc4', 'w', format='NETCDF4')
    ...
    for year in years:
        for month in months:
            ...
            for day in days:
                ...
                lat_hold, lon_hold, lev_hold, time_hold, tot_u_hold, tot_v_hold, t_hold = bam_merra_calc_d3.call_main(month, year, day)
                if lat_hold[0] == 'n':
